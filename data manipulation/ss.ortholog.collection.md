...
canis = useMart("ensembl", dataset="cfamiliaris_gene_ensembl") # 170704

## To see list of ensembl data ##
ensembl=useMart("ensembl")
listDatasets(ensembl) # To find database list
print(listAttributes(canis)$name) # To find attribute list
listFilters(canis)    # To find filter list
##-----------------------------##
...
# "with_homolog_scer" <- human
ortho_ensem = getLDS(attributes,filters="with_scerevisiae_homolog",
                     values=TRUE,mart=canis, # values=TRUE : all exist values
...
## Check data ##
Search = "hom.Sc.inpCANFA"
ls("package:hom.Sc.inp.db")[grep(Search,ls("package:hom.Sc.inp.db"))]
#as.list(hom.Hs.inpSACCE)[1:4] # What data in this?
head(as.list(hom.Sc.inpCANFA))
mget("ENSCAFP00000007145",hom.Sc.inpCANFA) # Example for catch a data
##------------##

library(DBI) # To utilize sql query

## Check data ##
print(dbListTables(hom.Sc.inp_dbconn())) # What species for orthologs?
dbListFields(hom.Sc.inp_dbconn(),"Canis_familiaris") # What columns in sql table
...
ortho_inpara = dbGetQuery(hom.Sc.inp_dbconn(),
                          "SELECT * FROM Canis_familiaris;")
...
ortho_inpara_yeast = which(ortho_inpara$species=="SACCE")
ortho_inpara_canis = which(ortho_inpara$species=="CANFA")
...
ortho_inpara_canis_id = which(ortho_inpara_names$Species=="CL_familiaris") # 170704
...
ortho_homo = homo.filter(homologene, tax1="4932",tax2="9615")
write.csv(ortho_homo, "ss.ortholog/ortho_homo.csv",row.names=F) # Yeast, Dog - 9.5 min
...
ortho_homo_canis_id = which(ortho_homo$TaxId=="9615")
...
ortho_union_egid_yeast = ss.venn3(ortho_egid_yeast, main="Human-Yeast orthologs (Yeast) from DBs")
  ...
  for(target in targets){
    ...
    if("ensembl" %in% dblist){
      ...
      if(Tbform==1 & length(ensem_rows>0)) {
        ...
        if(length(ensem_hu[,1])!=0){
          ensem_hu = data.frame(ensem_hu,Species="CL_familiaris",Group=paste0("q",i))
    ...
    if("homologene" %in% dblist){
      ...
      if(Tbform==1) {
        ...
        hu_id = which(homol_tmp$Species=="CL_familiaris")
