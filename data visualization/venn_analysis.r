...
venn_analysis = function(
    grouplist = NULL, # Input list variable
    out       = NULL, # Out folder path
    title     = '',   # Venn diagram title
    wfig      = TRUE, # Whether generating figure
    wfile     = TRUE, # Whether generating file
    verbose   = FALSE
) {
    ifelse(!dir.exists(out), dir.create(out),'')

    # Union list
    paste0('Union list = ') %>% cat
    g_names = names(grouplist)
    unionlist = Reduce(union,grouplist)
    length(unionlist) %>% print
